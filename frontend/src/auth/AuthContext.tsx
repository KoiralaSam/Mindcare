...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [wellnessCompleted, setWellnessCompleted] = useState<boolean>(
    initial?.wellnessCompleted ?? false,
  );

  const loginWithAPI = useCallback(
    async (body: { email: string; age?: number; gender?: string }) => {
      const result = await postLogin({
        email: body.email.trim().toLowerCase(),
        ...(body.age !== undefined ? { age: body.age } : {}),
        ...(body.gender !== undefined && body.gender !== ""
          ? { gender: body.gender }
          : {}),
      });
      if (!result.ok) {
